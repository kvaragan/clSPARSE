...
public:
    ...
    using sMatrixType = uBLAS::compressed_matrix<cl_float,  uBLAS::row_major, 0, uBLAS::unbounded_array<size_t> >;
    using dMatrixType = uBLAS::compressed_matrix<cl_double, uBLAS::row_major, 0, uBLAS::unbounded_array<size_t> >;
                             ...
                             context( context )
    {
        ...
        csrDMatrix.colIndices = ::clCreateBuffer( context, CL_MEM_READ_ONLY,
            csrDMatrix.num_nonzeros * sizeof(size_t), NULL, &status);

        csrDMatrix.rowOffsets = ::clCreateBuffer( context, CL_MEM_READ_ONLY,
            (csrDMatrix.num_rows + 1) * sizeof(size_t), NULL, &status);
        ...
        for (size_t i = 0; i < ublasDCsr.value_data().size(); i++)
            ublasSCsr.value_data()[i] = static_cast<double>(ublasDCsr.value_data()[i]);
        for (size_t i = 0; i < ublasDCsr.index1_data().size(); i++)
            ublasSCsr.index1_data()[i] = static_cast<size_t>(ublasDCsr.index1_data()[i]);
        for (size_t i = 0; i < ublasDCsr.index2_data().size(); i++)
            ublasSCsr.index2_data()[i] = static_cast<size_t>(ublasDCsr.index2_data()[i]);
    ...
    static size_t n_rows, n_cols, n_vals;
