            ...
            for (int i = 0; i < values.size(); i++)
{
    ...
    void test_csr_to_coo()
    {
        ...
        if (typeid(T) == typeid(cl_float))
        {
            ...
            size_t* rows = &CSRE::ublasSCsr.index1_data()[0];
            size_t* cols = &CSRE::ublasSCsr.index2_data()[0];

            size_t* coo_rows = new size_t[CSRE::n_vals];
            size_t* coo_cols = new size_t[CSRE::n_vals];
            float* coo_vals = new float[CSRE::n_vals];
            size_t total_vals = 0;
            for (size_t row = 0; row < CSRE::n_rows; row++)
            {
            ...
            std::vector<size_t> row_indices(cooMatrix.num_nonzeros);
            std::vector<size_t> col_indices(cooMatrix.num_nonzeros);
            std::vector<T> values(cooMatrix.num_nonzeros);

            // row indices
            cl_status = clEnqueueReadBuffer(CLSE::queue, cooMatrix.rowIndices,
                                            CL_TRUE, 0, row_indices.size() * sizeof(size_t),
            ...
            cl_status = clEnqueueReadBuffer(CLSE::queue, cooMatrix.colIndices,
                                            CL_TRUE, 0, col_indices.size() * sizeof(size_t),
                                            col_indices.data(), 0, nullptr, nullptr);
        ...
        if (typeid(T) == typeid(cl_double))
        {
            ...
            size_t* rows = &CSRE::ublasDCsr.index1_data()[0];
            size_t* cols = &CSRE::ublasDCsr.index2_data()[0];
