...
public:
    ...
    double bandwidth() // Need to modify this later **********
    {
        ...
        return (sizeof(size_t)*(csrMtx.num_nonzeros + csrMtx.num_rows) + sizeof(T) * (csrMtx.num_nonzeros + csrMtx.num_cols + csrMtx.num_rows)) / time_in_ns();
    } // end of function
...
private:
    ...
    size_t xSpMSpM_Getflopcount(void)
    {
        ...
        std::vector<size_t> colIdxA(nnzA, 0);
        std::vector<size_t> rowptrB (Browptrlen, 0);
        ...
        {
            size_t colIdx = colIdxA[i]; // Get colIdx of A
